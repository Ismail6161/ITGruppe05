...
class DB {
     ...
     */
    const SQL_INSERT = 'INSERT INTO artikel (id,c_ts,m_ts,Artikelnummer,Bezeichnung,Preis,Preis_c,bild) VALUES (?,?,?,?,?,?,?,?)'; // Nur als Beispiel, wird von Kindklasse überschrieben
    ...
    /**
     *  @var String Legt den Primärschlüssel einer Klasse fest, meist id
     */
    ...
    /**
     * Lädt anhand von <b><i>SQL_SELECT_PK</i></b> die Daten in das aktuelle Objekt. Defacto wird die <b><i>find()</i></b>-Methode und anschließend <b><i>import()</i></b> durchgeführt.<br>
     * Statt der id kann alternativ eine SQL-Anweisung mit anschließendem Parametern übergeben werden 
     * @param type $id Wert des Primärschlüssels (Feld in SQL-Anweisung angegeben)
     * @return boolean Gibt true oder false zurück.
     * @example <code>$result=$myObject->loadDBData(27);</code><br>Lädt in aktuelles Objekt Datensatz mit PK=27<br><code>$result=$myObject->loadDBData("SELECT * FROM User WHERE Kennung=?",["Nippa"]);</code><br>Lädt in aktuelles Objekt Ergebnis aus SQL. Beachten Sie, dass die Anweisung (ggf. mit LIMIT) auch nur einen Datensatz als Ergebnis liefert
     */
    ...
    /**
     * Statisch, gibt ein einzelnes <b>Objekt</b> der Klasse zurück anhand von <b><i>SQL_SELECT_PK</i></b><br>
     * Statt der id kann alternativ eine <b>SQL-Anweisung</b> mit anschließendem Parametern übergeben werden .
     * Also eigentlich überladen find($id) und find(SQL,[param1,....])
     * @param Long $id Wert des Primärschlüssels oder parametrisierte SQL-Anweisung
     * @param Array $param ordinales Array mit Werten(optional)
     * @return Object|false Standardobjekt oder false

     * @example <code>$stdobject=MyClass::find(27);</code>gibt üblicherweise den Datensatz mit dem Primärschlüssel 27 zurück<br><code>MyClass::find("SELECT * FROM User Where Kennung="?",[$variable]);</code>
     * 
     */
    public static function find($id, $param = array()) {
        if (count($param) != 0) {
            $SQL = $id; // Überladung verarbeiten
        } else {
    ...
    private static function restrictSQLOwned($SQL, &$param,$join=false,$group=false) {
        $SQLArray = DB::querySplit($SQL);
        $classname = static::class;
        // zunächst wir davon ausgegangen, dass kein PArameter nach WHERE folgt
    ...
    private static function restrictSQLExpression($SQL, $expr, &$param, $newparam = [], $join=false,$group=false) {
        $SQLArray = DB::querySplit($SQL);
        // zunächst wir davon ausgegangen, dass kein PArameter nach WHERE folgt und zunächst ohne Parameter
    ...
    /**
     * Legt aus dem aktuellen Objekt einen Datensatz an. Wenn nicht anders angegeben mit <b><i>SQL_INSERT</i></b>. Der erzeugte Primärschlüssel wird im Objekt aktualisiert.<br>Sollten durch SQL (Default-Werte, autotimestamp...) weitere eigene Werte erzeugt worden sein, wird ein erneutes <b><i>loadDBData()</i></b>empfohlen<br> Zeitstempel werden automatisch gesetzt.
     ...
     * @example <code>$result=$myObject->create()</code><br>Anhand von SQL_INSERT<br><code>$result=$myObject->create("INSERT INTO...VALUES(?,?)")</code><br>Attributwerte werden übernommen<br><code>$result=$myObject->create("INSERT INTO...VALUES(?,?)",[$var1,$var2])</code><br>Werte separat übergeben
    ...
    public function create($SQL = "", $param = array()) {
        ...
        if ($param[0] == "") { // Dann zieht er sich automatisch die Feldnamen aus der Insert Anweisung
            ...
            if (static::$access == true) {
                ...
                $SQL = str_replace("?)", "?, ?, ?, ?)", $SQL); // Parameter ergänzen in SQL
     ...
     * @return boolean
     * @example <code>$result=myObject->update()</code> anhand SQL_UPDATE<br><code>$result=myObject->update("UPDATE User SET Vorname=?")</code><br>Übernimmt automatisch Attributwert Vorname<br><code> myObject->update("UPDATE User Set Vorname=?",["Klaus"])</code><br>Aktualisiert die Tabelle direkt mit konkreten (Variablen-)Werten
     */
     ...
     * @param Long $id Primärschlüsselwert des zu Löschenden Datensatzes auf Grundlage <b><i>SQL_DELETE_PK</i></b><br>Alternativ SQL-Abfrage+Parameter
     * @param Array $param Parameterwerte für SQL-Anweisung
     * @return boolean
    ...
    /**
     * Führt ein Prepared-SQL-Statement aus(PDO). Es sollte sich um eine Auswahlabfrage handeln, die ein oder mehrere Elemente zurückliefert.<br>Sie werden als Standardobjekte in einem Array zurückgegeben.<br>Standardobjekte beinhalten alle Attribute der Abfrage, aber keine Methoden etc.
     ...
     * @example <code>$object->import($stdobject)</code><br>Lädt Standardobjekt als vollwertige Instanz
     */
    ...
    /**
     * Aktualisiert/erstellt eine Datei in DB und im Filesystem.<br> Standardmäßig wird ein Zufallsdateiname generiert.<br>Über <i>options</i> können weitere Einstellungen vorgenommen werden
     * @param String $attribut Attribut/Feldname der Datei, die in der DB nur den Pfad enthält.
    ...
    public function updateFile($attribut, $options = []) {
        ...
        if ($file['error'] == UPLOAD_ERR_OK) { // Dateiübertragung zum Server erfolgreich
            $endung = pathinfo($file['name'], PATHINFO_EXTENSION);
            $ftypes = explode(";", $opt["filetypes"]);
            if (!in_array($endung, $ftypes)) { // zulässiger Dateityp??
            ...
            if (is_file($fullPath)) {
                if ($opt['overwrite'] == "true") {
                    Core::addMessage("Datei überschrieben");
                } else {
            ...
            switch ($file['error']) {
                case UPLOAD_ERR_NO_FILE:
                    if ($opt['mandatory'] == "false") {
                        ...
                        core::addError("Es wurde keine Datei ausgewählt");
                        return false;
                    }
                    break;
                ...
                case UPLOAD_ERR_NO_TMP_DIR:
                    core::addError("Kein temporäres Serververziechnis vorhanden");
                    return false;
                    break;
                ...
                case UPLOAD_ERR_INI_SIZE:
                    core::addError("Datei überschreitet maximale Dateigröße");
                    return false;
                    break;
                case UPLOAD_ERR_FORM_SIZE:
                    core::addError("Datei überschreitet im Formular definierte Dateigröße");
                    return false;
                    break;
    ...
    /**
     * Prüft alle Attribute Anhand des vorgebebenen Schemas<br> 
     * 
     */
        ...
        foreach (static::$dataScheme as $att => $value) {
            switch ($value["type"]) {
                case static::TYPE_PRIMARY:
                    if ($strict && !isset($this->$att) && $value["null"] === false) { // Nullwerte bei Bedarf prüfen
                ...
                case static::TYPE_INTEGER:
                    if ($strict && !isset($this->$att) && $value["null"] === false) { // Nullwerte bei Bedarf prüfen
                        ...
                        } else { // zusätzliche Optionen prüfen
                            if (isset($value["min"])) {
    ...
    Public function format_old($viewport = "list") {
        foreach (static::$dataScheme as $att => $value) {
            ...
            // Einheit ergänzen nur bei action="show"
            if ($action == "show") {
                if (isset($value["unit"])) {
    ...
    public function renderLabel($attr, $viewport = "") {
        ...
        if (isset($scheme["display"])) {   // wenn Attribut nicht angezeigt werden soll Rendern abbrechen
            if ($scheme["display"] === "false" || $renderAs["display"] === false) {
                return false;
            }
        }
        ...
        if (isset($renderAs["display"])) { // Abbruch falls Element in diesem Modus nicht gerendert werden soll
            if ($renderAs["display"] === "false" || $renderAs["display"] === false) { //|| $renderAs['control']==="checkbox"){
                return false;
            }
        }
    ...
    /*
     * Rendert das Label als Überschrift (z.B. einer Tabelle)nach der Angabe des Labels im Datascheme
     * @param String $attr Name des Attributs
     * @param String $html Die Ausgabe des Header erfolgt inklusive HTML oder nur als Text<br>Mögliche Werte: "none" und "table"
     * @return boolean Gibt true oder false zurück.
    ...
    public function renderHeader($attr, $html = "none") {
        ...
        $renderAs = $scheme["renderAs"][$viewport];
        if (isset($renderAs["display"])) { // Abbruch falls Element in diesem Modus nicht gerendert werden soll
            if ($renderAs["display"] === "false" || $renderAs["display"] === false) {
                return false;
            }
        }
    ...
    public function render($attr, $viewport = "", $as = "") {
        ...
        if (isset($scheme["display"])) {   // wenn Attribut nicht angezeigt werden soll Rendern abbrechen
            if ($scheme["display"] == "false" || $scheme["display"] === false) {
                return false;
            }
        }
            ...
            Case "primitive":
                ...
                switch ($control) {
                    ...
                    case "password":
                        ...
                        echo snippet::password($attr, $value, "", $renderAs["disabled"], $renderAs["readonly"], $doublecheck);
                        //  include("includes/getSnippet/$control.php");
                        break;
                    case "hidden":
                        $value = $this->format($attr);
                    ...
                    case "textarea":
                        $value = $this->format($attr);
                        echo snippet::textArea($attr, $value, $renderAs["class"], $renderAs["disabled"], $renderAs["readonly"]);
                    ...
                    case "datetime":
                        ...
                        $value = str_replace(" ", "T", $value); // Großes T für Steuerelement einfügen
                        echo snippet::dateTime($attr, $value, "", $renderAs["disabled"], $renderAs["readonly"]);
                        break;
                    case "time":
                        // Großes T für Steuerelement einfügen
                        echo snippet::time($attr, $value, "", $renderAs["disabled"]);
                        break;
                    case "flipswitch":
                        // Großes T für Steuerelement einfügen
                        $value = $this->format($attr);
                        echo snippet::flipswitch($attr, $value, "", $renderAs["disabled"]);
                        break;
                    case "checkbox":
                        $value = $this->format($attr);

                        echo snippet::checkbox($attr, $value, $label, "", $renderAs["disabled"]);
                        break;
                    case "option":
                        break;
                    case "link":
                    ...
                    case "picture":
                        ...
                        if ($viewport == "list") {
                            ...
                            $mode = "form"; // hier muss das noch viewportabhängig aus dem datascheme gelesen werden 
                        }

                        echo snippet::picture($attr, $src, $class, $mode);
                        break;
                    case "slider":
                        $value = $this->format($attr);
                    ...
                    case "text":
                        ...
                        switch ($viewport) {
                            case "detail":
                                ...
                                break;
                            default:
                                echo Snippet::text($value, "plain", $attr);
                        }
                ...
                switch ($control) {
                    ...
                    case "menu":
                        if (!isset($scheme["identifier"])) {
                            ...
                            $ident = $scheme["identifier"]; // GEHT NUR SO, DA NICHT bekannt ist, für welche Klasse das Element agiert.
                        ...
                        $arrayobj = Core::import($attr);
                         $readonly = $renderAs["readonly"];
                            $disabled = $renderAs["disabled"];
                        $output = Snippet::selectmenu($arrayobj, ["default" => $value, "name" => $attr, "text" => $ident,"readonly" => $readonly,"disabled" => $disabled]); //Bindung an Element
                        echo $output;
                        //   include("includes/getSnippet/$control.php");
                        break;
...
/*                if (isset($dataScheme[$key]['classAlias'])){
    ...
    public function buildScheme($class, $stereotyp = "class") {
        ...
        // Step 2 Konkretes DataScheme hinzufügen, maßgeblich um alle Attribute zu handeln, überschreibt ggf. default-Werte Step 1

        $newScheme2 = self::array_cascade([$newScheme, $dataSchemeConcrete]);

        // Step 3 Jedes Attribut durchgehen und Anhand des Datentyps zugehöriges DefaultScheme laden
