...
const SQL_SELECT_Kunde_uid='select User.id as id, User.Passwort as Passwort, `GruppeT0`.`literal` as `Gruppe_literal`, `User`.`Gruppe` as `Gruppe`, User.Kennung as Kennung, User.roleid as roleid from User left join `GruppeT` as GruppeT0 on `User`.`Gruppe` = `GruppeT0`.`id`  where `User`.`id` = (select _User_uid from Kunde where id=?)';
...
public function login(){
    ...
    $param=[$this->Kennung,$this->Passwort];
