...
class Core {
    ...
    private static function write_protocol($task, $datensatz_id, $User_uid) {
        $explode = explode("_", $task);
        if (count($explode) == 1) {
            $label = ucfirst($explode[0]) . " Übersicht";
            ...
            if ($add == "") {
                $label = ucfirst($task) . " Übersicht";
            } else {
                $label = ucfirst($explode[0]) . $add;
            }
        } else {
            ...
            if ($add == "") {
                ...
                $label = ucfirst($label) . $add;
            }
        }
        if ($explode[0] != "home" && $explode[0] != "" && $explode[0] != "favoriten" && $explode[0] != "protokoll" && $explode[0] != "login" && $explode[0] != "logout") {
            $db = Core::$pdo;

            $SQL = "SELECT * FROM Aktivitätenprotokoll WHERE User_uid=?";
            $stmt = $db->prepare($SQL);
            $result = $stmt->execute([$User_uid]);
            $daten = $stmt->fetchAll(PDO::FETCH_CLASS);
            if (count($daten) >= 10) {
                $x = 0;
                foreach ($daten as $item) {
                    $SQL = "DELETE FROM Aktivitätenprotokoll WHERE id=?";
                    $stmt = $db->prepare($SQL);
            ...
            $SQL = "SELECT * FROM Aktivitätenprotokoll WHERE m_ts = (Select Max(m_ts) From Aktivitätenprotokoll)";
            $stmt = $db->prepare($SQL);
            $result = $stmt->execute();
            $daten = $stmt->fetchAll(PDO::FETCH_CLASS);
            ...
            $SQL = "INSERT INTO Aktivitätenprotokoll (task,task_label,datensatz_id,User_uid) VALUES (?,?,?,?)";
            $stmt = $db->prepare($SQL);
            $result = $stmt->execute([$task, $label, $datensatz_id, $User_uid]);
            return $result;
        }
    }
    ...
    public static function controller() {
        ...
        if (null != array_key_exists(self::$task, self::$taskMap)) {
            ...
            Core::write_protocol($task, $datensatz_id, $_SESSION['uid']);
    ...
    public static function publish($controllervar, $varname) {
